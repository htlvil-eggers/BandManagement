...
public class WebserviceManager {
	...
	public static void updateMusician(Musician musician) {
		service.path("rest").path("musicians").accept(MediaType.TEXT_XML).put(musician);
		
	}
	
	public static void addAvailableTime(String bandname, String username, Date from, Date to) {
		service.path("rest").path("availableTimes").accept(MediaType.TEXT_XML).post(new AvailableTimeWrapper(bandname, username, from, to));
	}
	...
	public static void answerAppearanceRequest(Appointment selectedAppearance, String bandname, String username,
		...
		service.path("rest").path("appearanceRequests").accept(MediaType.TEXT_XML).put(new AppearanceRequestWrapper(selectedAppearance, bandname, username, password, accepted));
	}
}
